...
public class AudioRecorderUtils {
    ...
    public static AudioRecorderUtils getInstanse(Boolean recordingCompressed) {
        ...
        AudioRecorderUtils result = null;
        if (recordingCompressed) {
            result = new AudioRecorderUtils(false, AudioSource.MIC,sampleRates[3], AudioFormat.CHANNEL_IN_MONO,
                    AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 0;
            do {
                result = new AudioRecorderUtils(true, AudioSource.MIC, sampleRates[i], AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT);
        ...
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) { // Check to make sure
                    // buffer size is not smaller than the smallest allowed one
            ...
            if (state == State.INITIALIZING) {
                if (rUncompressed) {
                    if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
                        ...
                        randomAccessWriter.writeShort(Short.reverseBytes((short) (nChannels * bSamples / 8))); // Block lign, NumberOfChannels*BitsPerSample/8
